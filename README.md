...
* Do not listen to the "EULYNX Dataprep is obsolete" misrepresentations. While EULYNX DP is not currently maintained, it is recent, and signalling concepts (e.g. those concerning class B systems) will probably remain valid for a longer time than any technical system.
* Do not trust derived models, especially when the derivation is untraceable, undocumented, and (quite unlikely) in breach of licensing terms. The level of detail of the original EULYNX DP documentation has yet to be matched.
